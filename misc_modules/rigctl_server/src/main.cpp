...
private:
    static void menuHandler(void* ctx) {
        ...
        if (listening && ImGui::Button(CONCAT("停止录制##_rigctl_srv_stop_", _this->name), ImVec2(menuWidth, 0))) {
            _this->stopServer();
        }
        else if (!listening && ImGui::Button(CONCAT("开始录制##_rigctl_srv_stop_", _this->name), ImVec2(menuWidth, 0))) {
            _this->startServer();
        }
