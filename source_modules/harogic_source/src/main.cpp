...
private:
    ...
    static void start(void* ctx) {
        ...
        flog::debug("得到缓冲区大小: {}", _this->bufferSize);

        // Start the stream
        ret = IQS_BusTriggerStart(&_this->openDev);
        if (ret < 0) {
            flog::error("无法启动流数据传输: {}", ret);
            Device_Close(&_this->openDev);
            return;
        }
    ...
    void worker() {
        ...
        flog::debug("每次将交换 {} 个缓冲区。", bufCount);
