...
private:
    ...
    static void menuSelected(void* ctx) {
        ...
        flog::info("RFNMSourceModule '{0}': 菜单选择!", _this->name);
    }

    static void menuDeselected(void* ctx) {
        RFNMSourceModule* _this = (RFNMSourceModule*)ctx;
        flog::info("RFNMSourceModule '{0}': 菜单取消选择!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        _this->running = true;
        flog::info("RFNMSourceModule '{0}': 启动!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("RFNMSourceModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        if (_this->running) {
            ...
            rfnm_api_failcode fail = _this->openDev->set(_this->currentPath.appliesCh);
            if (fail != rfnm_api_failcode::RFNM_API_OK) {
                flog::error("调谐失败: {}", (int)fail);
            }
        }
        _this->freq = freq;
        flog::info("RFNMSourceModule '{0}': 调谐: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (SmGui::Checkbox(CONCAT("调频陷波##_rfnm_", _this->name), &_this->fmNotch)) {
            if (_this->running) {
                _this->openDev->s->rx.ch[_this->currentPath.chId].fm_notch = _this->fmNotch ? rfnm_fm_notch::RFNM_FM_NOTCH_ON : rfnm_fm_notch::RFNM_FM_NOTCH_OFF;
                rfnm_api_failcode fail = _this->openDev->set(_this->currentPath.appliesCh);
            }
