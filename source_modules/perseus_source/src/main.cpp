...
public:
    ...
    void refresh() {
        ...
        if (devCount < 0) {
            libInit = false;
            flog::error("无法初始化 libperseus: {}", perseus_errorstr());
            return;
        }
        libInit = true;
        ...
        for (int i = 0; i < devCount; i++) {
            ...
            if (!dev) {
                flog::error("无法打开 Perseus 设备，ID {}: {}", i, perseus_errorstr());
                continue;
            }
            ...
            if (err) {
                flog::error("无法上传固件到设备 {}: {}", i, perseus_errorstr());
                perseus_close(dev);
                continue;
            }
            ...
            if (err) {
                flog::error("无法从设备 {}: {}", i, perseus_errorstr());
                perseus_close(dev);
                continue;
            }
    ...
    void select(const std::string& serial) {
        ...
        if (!dev) {
            flog::error("无法打开设备 {}: {}", selectedPerseusId, perseus_errorstr());
            selectedSerial.clear();
            return;
        }
        ...
        int err = perseus_firmware_download(dev, NULL);
        if (err) {
            flog::error("无法上传固件到设备: {}", perseus_errorstr());
            perseus_close(dev);
            selectedSerial.clear();
            return;
        }
        ...
        err = perseus_get_product_id(dev, &prodId);
        if (err) {
            flog::error("无法从设备获取产品信息: {}", perseus_errorstr());
            perseus_close(dev);
            selectedSerial.clear();
            return;
        }
        ...
        if (err) {
            flog::error("无法获取采样率列表: {}", perseus_errorstr());
            perseus_close(dev);
            selectedSerial.clear();
            return;
        }
...
private:
    ...
    static void start(void* ctx) {
        ...
        if (!_this->openDev) {
            flog::error("无法打开设备 {}: {}", _this->selectedPerseusId, perseus_errorstr());
            return;
        }
        ...
        if (err) {
            flog::error("无法设置采样率: {}", perseus_errorstr());
            perseus_close(_this->openDev);
            return;
        }
        ...
        if (err) {
            flog::error("无法启动流: {}", perseus_errorstr());
            perseus_close(_this->openDev);
            return;
        }

        _this->running = true;
        flog::info("PerseusSourceModule '{0}': 启动!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("PerseusSourceModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        _this->freq = freq;
        flog::info("PerseusSourceModule '{0}': 调谐: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (SmGui::Checkbox(CONCAT("前置放大器##_airspyhf_preamp_", _this->name), &_this->preamp)) {
            if (_this->running) {
                perseus_set_adc(_this->openDev, _this->dithering, _this->preamp);
            }
            if (!_this->selectedSerial.empty()) {
        ...
        if (SmGui::Checkbox(CONCAT("预选##_airspyhf_presel_", _this->name), &_this->preselector)) {
            if (_this->running) {
                perseus_set_ddc_center_freq(_this->openDev, _this->freq, _this->preselector);
            }
            if (!_this->selectedSerial.empty()) {
