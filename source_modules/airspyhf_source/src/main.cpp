...
#else
            flog::warn("==== 调用 airspyhf_open_fd ====");
            int err = airspyhf_open_fd(&dev, devFd);
            flog::warn("==== 调用 airspyhf_open_fd => ({0}) ====", err);
#endif
            if (err != 0) {
                char buf[1024];
                sprintf(buf, "%016" PRIX64, serial);
                flog::error("无法打开 Airspy HF+ {0}", buf);
                selectedSerial = 0;
                return;
            }
        }
...
private:
    ...
    static void start(void* ctx) {
        ...
        if (_this->selectedSerial == 0) {
            flog::error("尝试使用空序列号启动 AirspyHF+ 源模块");
            return;
        }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (SmGui::Checkbox(CONCAT("高频低噪声放大器##_airspyhf_lna_", _this->name), &_this->hfLNA)) {
            if (_this->running) {
                airspyhf_set_hf_lna(_this->openDev, _this->hfLNA);
            }
            if (_this->selectedSerStr != "") {
