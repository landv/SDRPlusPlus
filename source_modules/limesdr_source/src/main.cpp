...
private:
    ...
    int getBestBandwidth(int sampleRate) {
        for (int i = 0; i < bandwidths.size(); i++) {
            if (bandwidths[i] >= sampleRate) {
                flog::warn("所选带宽为 {0}", bandwidths[i]);
                return bandwidths[i];
            }
        }
    ...
    static void start(void* ctx) {
        ...
        if (err) {
            ...
            if (err = LMS_Init(_this->openDev)) {
                flog::error("无法重新初始化设备 ({})", err);
                return;
            }
        }

        flog::warn("通道数量: {0}", LMS_GetNumChannels(_this->openDev, false));
        ...
        _this->running = true;
        flog::info("LimeSDRSourceModule '{0}': 开始！", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("LimeSDRSourceModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        flog::info("LimeSDRSourceModule '{0}': 调谐: {1}!", _this->name, freq);
    }
