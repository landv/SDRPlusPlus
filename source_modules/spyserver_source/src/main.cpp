...
private:
    ...
    static void menuSelected(void* ctx) {
        ...
        flog::info("SpyServerSourceModule '{0}': 菜单选择!", _this->name);
    }

    static void menuDeselected(void* ctx) {
        SpyServerSourceModule* _this = (SpyServerSourceModule*)ctx;
        gui::mainWindow.playButtonLocked = false;
        flog::info("SpyServerSourceModule '{0}': 菜单取消选择!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        _this->running = true;
        flog::info("SpyServerSourceModule '{0}': 启动!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        _this->running = false;
        flog::info("SpyServerSourceModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        _this->freq = freq;
        flog::info("SpyServerSourceModule '{0}': 调谐: {1}!", _this->name, freq);
    }
    ...
    void tryConnect() {
        try {
            ...
            flog::error("无法连接到 spyserver {}", e.what());
        }
    }
