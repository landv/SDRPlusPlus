...
public:
    ...
    void refresh() {
        ...
        for (int i = 0; i < devCount; i++) {
            ...
            if (venBuf[0] && prodBuf[0]) {
                sprintf(buf, "%s %s [%s]##%d", venBuf, prodBuf, (!snErr && snBuf[0]) ? snBuf : "无序列号", i);
            }
            else {
                sprintf(buf, "%s [%s]##%d", devName, (!snErr && snBuf[0]) ? snBuf : "无序列号", i);
            }
    ...
    void selectById(int id) {
        ...
        if (oret < 0) {
            selectedDevName = "";
            flog::error("无法打开 RTL-SDR: {0}", oret);
            return;
        }
...
private:
    ...
    static void menuSelected(void* ctx) {
        ...
        flog::info("RTLSDRSourceModule '{0}': 菜单选择!", _this->name);
    }

    static void menuDeselected(void* ctx) {
        RTLSDRSourceModule* _this = (RTLSDRSourceModule*)ctx;
        flog::info("RTLSDRSourceModule '{0}': 菜单取消选择!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        if (oret < 0) {
            flog::error("无法打开 RTL-SDR");
            return;
        }

        flog::info("RTL-SDR 采样率: {0}", _this->sampleRate);
        ...
        _this->running = true;
        flog::info("RTLSDRSourceModule '{0}': 启动!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("RTLSDRSourceModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        _this->freq = freq;
        flog::info("RTLSDRSourceModule '{0}': 调谐: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (SmGui::Checkbox(CONCAT("偏置 T##_rtlsdr_rtl_biast_", _this->name), &_this->biasT)) {
            if (_this->running) {
                rtlsdr_set_bias_tee(_this->openDev, _this->biasT);
            }
            if (_this->selectedDevName != "") {
        ...
        if (SmGui::Checkbox(CONCAT("偏移调整##_rtlsdr_rtl_ofs_", _this->name), &_this->offsetTuning)) {
            if (_this->running) {
                rtlsdr_set_offset_tuning(_this->openDev, _this->offsetTuning);
            }
            if (_this->selectedDevName != "") {
        ...
        if (SmGui::Checkbox(CONCAT("RTL自动增益控制##_rtlsdr_rtl_agc_", _this->name), &_this->rtlAgc)) {
            if (_this->running) {
                rtlsdr_set_agc_mode(_this->openDev, _this->rtlAgc);
            }
            if (_this->selectedDevName != "") {
        ...
        SmGui::ForceSync();
        if (SmGui::Checkbox(CONCAT("调谐器AGC##_rtlsdr_tuner_agc_", _this->name), &_this->tunerAgc)) {
            if (_this->running) {
                if (_this->tunerAgc) {
                    rtlsdr_set_tuner_gain_mode(_this->openDev, 0);
                }
