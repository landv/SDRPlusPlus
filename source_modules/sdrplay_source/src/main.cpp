...
public:
    SDRPlaySourceModule(std::string name) {
        ...
        if (err != sdrplay_api_Success) {
            flog::error("SDRplay API 初始化失败。请确保该服务正在运行。");
            return;
        }
    ...
    void selectDev(sdrplay_api_DeviceT dev, int id) {
        ...
        if (err != sdrplay_api_Success) {
            ...
            flog::error("无法选择 RSP 设备: {0}", errStr);
            selectedName = "";
            return;
        }
        ...
        if (err != sdrplay_api_Success) {
            const char* errStr = sdrplay_api_GetErrorString(err);
            flog::error("无法获取 RSP 设备参数: {0}", errStr);
            selectedName = "";
            return;
        }
        ...
        if (err != sdrplay_api_Success) {
            const char* errStr = sdrplay_api_GetErrorString(err);
            flog::error("无法初始化 RSP 设备: {0}", errStr);
            selectedName = "";
            return;
        }
...
private:
    ...
    static void menuSelected(void* ctx) {
        ...
        flog::info("SDRPlaySourceModule '{0}': 菜单选择!", _this->name);
    }

    static void menuDeselected(void* ctx) {
        SDRPlaySourceModule* _this = (SDRPlaySourceModule*)ctx;
        flog::info("SDRPlaySourceModule '{0}': 菜单取消选择!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        _this->running = true;
        flog::info("SDRPlaySourceModule '{0}': 启动!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("SDRPlaySourceModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        _this->freq = freq;
        flog::info("SDRPlaySourceModule '{0}': 调谐: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (_this->selectedName != "") {
            ...
            SmGui::ForceSync();
            if (SmGui::Checkbox(CONCAT("中频自动增益控制##sdrplay_agc", _this->name), &_this->agc)) {
                if (_this->running) {
                    _this->channelParams->ctrlParams.agc.enable = _this->agc ? sdrplay_api_AGC_CTRL_EN : sdrplay_api_AGC_DISABLE;
                    if (_this->agc) {
                        _this->channelParams->ctrlParams.agc.attack_ms = _this->agcAttack;
            ...
            SmGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "没有可用的设备");
        }
    }
    ...
    void RSP1AMenu() {
        if (SmGui::Checkbox(CONCAT("调频/中波陷波##sdrplay_rsp1a_fmmwnotch", name), &rsp1a_fmmwNotch)) {
            if (running) {
        ...
        if (SmGui::Checkbox(CONCAT("DAB 缺口##sdrplay_rsp1a_dabnotch", name), &rsp1a_dabNotch)) {
            if (running) {
        ...
        if (SmGui::Checkbox(CONCAT("偏置-T##sdrplay_rsp1a_biast", name), &rsp1a_biasT)) {
            if (running) {
    ...
    void RSP2Menu() {
        ...
        if (rsp2_antennaPort != 2) {
            if (SmGui::Checkbox(CONCAT("中波/调频陷波##sdrplay_rsp2_fmmwnotch", name), &rsp2_fmmwNotch)) {
                if (running) {
            ...
            style::beginDisabled();
            bool dummy = false;
            SmGui::Checkbox(CONCAT("中波/调频陷波##sdrplay_rsp2_fmmwnotch", name), &dummy);
            style::endDisabled();
        }
        
        if (SmGui::Checkbox(CONCAT("偏置-T##sdrplay_rsp2_biast", name), &rsp2_biasT)) {
            if (running) {
    ...
    void RSPduoMenu() {
        ...
        if (SmGui::Checkbox(CONCAT("中波/调频陷波##sdrplay_rspduo_fmmwnotch", name), &rspduo_fmmwNotch)) {
            if (running) {
        ...
        if (SmGui::Checkbox(CONCAT("DAB 缺口##sdrplay_rspduo_dabnotch", name), &rspduo_dabNotch)) {
            if (running) {
        ...
        if (SmGui::Checkbox(CONCAT("偏置-T##sdrplay_rspduo_biast", name), &rspduo_biasT)) {
            if (running) {
    ...
    void RSPdxMenu() {
        ...
        if (SmGui::Checkbox(CONCAT("中波/调频陷波##sdrplay_rspdx_fmmwnotch", name), &rspdx_fmmwNotch)) {
            if (running) {
        ...
        if (SmGui::Checkbox(CONCAT("DAB 缺口##sdrplay_rspdx_dabnotch", name), &rspdx_dabNotch)) {
            if (running) {
        ...
        if (SmGui::Checkbox(CONCAT("偏置-T##sdrplay_rspdx_biast", name), &rspdx_biasT)) {
            if (running) {
    ...
    void RSPUnsupportedMenu() {
        SmGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "该设备目前不受支持。");
    }
