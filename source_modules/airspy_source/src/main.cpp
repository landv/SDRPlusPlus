...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        if (_this->gainMode == 0) {
            ...
            // AGC Control
            SmGui::ForceSync();
            if (SmGui::Checkbox(CONCAT("低噪声放大器自动增益控制##_airspy_", _this->name), &_this->lnaAgc)) {
                if (_this->running) {
                    if (_this->lnaAgc) {
                        airspy_set_lna_agc(_this->openDev, 1);
                    }
            ...
            SmGui::ForceSync();
            if (SmGui::Checkbox(CONCAT("混频器AGC##_airspy_", _this->name), &_this->mixerAgc)) {
                if (_this->running) {
                    if (_this->mixerAgc) {
                        airspy_set_mixer_agc(_this->openDev, 1);
                    }
