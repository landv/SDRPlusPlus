...
private:
    ...
    static void start(void* ctx) {
        ...
        if (_this->selectedSerial == "") {
            flog::error("尝试使用空串口启动 HackRF 源码。");
            return;
        }
        ...
        if (err != HACKRF_SUCCESS) {
            flog::error("无法打开 HackRF {0}: {1}", _this->selectedSerial, hackrf_error_name(err));
            return;
        }
    ...
    static void stop(void* ctx) {
        ...
        if (err != HACKRF_SUCCESS) {
            flog::error("无法关闭 HackRF {0}: {1}", _this->selectedSerial, hackrf_error_name(err));
        }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (SmGui::Checkbox(CONCAT("偏置-T##_hackrf_bt_", _this->name), &_this->biasT)) {
            if (_this->running) {
                hackrf_set_antenna_enable(_this->openDev, _this->biasT);
            }
            config.acquire();
