    ...
    void selectBySerial(uint64_t serial) {
        ...
        try {
            ...
            if (err != 0) {
                ...
                flog::error("无法打开 HydraSDR {0}", buf);
                selectedSerial = 0;
                return;
            }
        }
...
private:
    ...
    static void start(void* ctx) {
        ...
        if (_this->selectedSerial == 0) {
            flog::error("尝试使用空串口启动 HydraSDR 源");
            return;
        }
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::Button(CONCAT("刷新##_hydrasdr_refr_", _this->name))) {
            _this->refresh();
            config.acquire();
            std::string devSerial = config.conf["device"];
            config.release();
        ...
        if (_this->gainMode == 0) {
            ...
            // AGC Control
            SmGui::ForceSync();
            if (SmGui::Checkbox(CONCAT("低噪声放大器自动增益控制##_hydrasdr_", _this->name), &_this->lnaAgc)) {
                if (_this->running) {
                    if (_this->lnaAgc) {
                        hydrasdr_set_lna_agc(_this->openDev, 1);
                    }
            ...
            SmGui::ForceSync();
            if (SmGui::Checkbox(CONCAT("混频器自动增益控制##_hydrasdr_", _this->name), &_this->mixerAgc)) {
                if (_this->running) {
                    if (_this->mixerAgc) {
                        hydrasdr_set_mixer_agc(_this->openDev, 1);
                    }
        ...
        // Bias T
        if (SmGui::Checkbox(CONCAT("偏置 T##_hydrasdr_", _this->name), &_this->biasT)) {
            if (_this->running) {
                hydrasdr_set_rf_bias(_this->openDev, _this->biasT);
            }
            if (_this->selectedSerStr != "") {
