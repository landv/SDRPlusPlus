...
private:
    static void menuSelected(void* ctx) {
        ...
        flog::info("RTLTCPSourceModule '{0}': 菜单选择!", _this->name);
    }

    static void menuDeselected(void* ctx) {
        RTLTCPSourceModule* _this = (RTLTCPSourceModule*)ctx;
        flog::info("RTLTCPSourceModule '{0}': 菜单取消选择!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        try {
            ...
            flog::error("可以连接到RTL-TCP服务器： {}", e.what());
            return;
        }
        ...
        _this->running = true;
        flog::info("RTLTCPSourceModule '{0}': 启动!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        _this->client->close();
        _this->running = false;
        flog::info("RTLTCPSourceModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        _this->freq = freq;
        flog::info("RTLTCPSourceModule '{0}': 调谐: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::FillWidth();
        if (SmGui::Combo(CONCAT("##_rtltcp_ds_", _this->name), &_this->directSamplingId, "禁用\0I分支\0Q分支\0")) {
            if (_this->running) {
                _this->client->setDirectSampling(_this->directSamplingId);
                _this->client->setGainIndex(_this->gain);
            }
        ...
        if (SmGui::Checkbox(CONCAT("偏置-T##_biast_select_", _this->name), &_this->biasTee)) {
            if (_this->running) {
                _this->client->setBiasTee(_this->biasTee);
            }
            config.acquire();
        ...
        if (SmGui::Checkbox(CONCAT("偏移调整##_biast_select_", _this->name), &_this->offsetTuning)) {
            if (_this->running) {
                _this->client->setOffsetTuning(_this->offsetTuning);
            }
            config.acquire();
        ...
        if (SmGui::Checkbox("RTL自动增益控制", &_this->rtlAGC)) {
            if (_this->running) {
                _this->client->setAGCMode(_this->rtlAGC);
                if (!_this->rtlAGC) {
                    _this->client->setGainIndex(_this->gain);
        ...
        SmGui::ForceSync();
        if (SmGui::Checkbox("调谐器AGC", &_this->tunerAGC)) {
            if (_this->running) {
