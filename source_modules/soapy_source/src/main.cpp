...
private:
    ...
    float selectBwBySr(double samplerate) {
        ...
        flog::info("采样率{0}的带宽为{1}", samplerate, cur);
        return cur;
    }

    void selectSampleRate(double samplerate) {
        flog::info("设置采样率为{0}", samplerate);
    ...
    static void menuSelected(void* ctx) {
        ...
        flog::info("SoapyModule '{0}': 菜单选择!", _this->name);
    ...
    static void menuDeselected(void* ctx) {
        SoapyModule* _this = (SoapyModule*)ctx;
        flog::info("SoapyModule '{0}': 菜单取消选择!", _this->name);
    }
    ...
    static void start(void* ctx) {
        ...
        flog::info("SoapyModule '{0}': 启动!", _this->name);
    }
    ...
    static void stop(void* ctx) {
        ...
        flog::info("SoapyModule '{0}': 停止!", _this->name);
    }
    ...
    static void tune(double freq, void* ctx) {
        ...
        flog::info("SoapyModule '{0}': 协调: {1}!", _this->name, freq);
    }
    ...
    static void menuHandler(void* ctx) {
        ...
        if (_this->hasAgc) {
            if (SmGui::Checkbox((std::string("自动增益控制##_agc_sel_") + _this->name).c_str(), &_this->agc)) {
